...
def render_polling_experience() -> None:
    ...
    if reports:
        ...
        with c1: st.metric(T("Total Reports"),       len(reports),                   help=T("Number of voter reports submitted"))
        with c2: st.metric(T("Avg Queue Time"),      f"{avg_queue} {T('min')}",      help=T("Average wait time reported"))
        with c3: st.metric(T("Accessible Booths"),   f"{accessible_count}/{len(reports)}", help=T("Booths reported as wheelchair accessible"))
        with c4: st.metric(T("Positive Experience"), f"{excellent}/{len(reports)}",  help=T("Reports marked Excellent or Smooth"))
    ...
    for report in all_reports:
        ...
        acc_badge = (
            f'<span style="background:#E8F5E6;color:#0E6B06;font-size:0.68rem;font-weight:700;'
            f'padding:2px 8px;border-radius:20px;border:1px solid #B8E0B4;">♿ {accessible_label}</span>'
            if report["accessible"] else
            f'<span style="background:#FFEBEE;color:#C62828;font-size:0.68rem;font-weight:700;'
            f'padding:2px 8px;border-radius:20px;border:1px solid #FF8A80;">⚠️ {accessibility_issue}</span>'
        )
        queue_color = (
        ...
        st.markdown(
            f"""
            <div style="background:#1C2030;border-radius:14px;padding:16px;
                ...
                <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px;">
                    <span style="background:{queue_color}12;color:{queue_color};font-size:0.72rem;
                        ...
                        ⏱️ {report['queue_minutes']} {min_wait_label}
                    </span>
                    {acc_badge}
                </div>
